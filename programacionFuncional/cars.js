...
// List
const list = cars.map(element => `This ${element.brand} from ${element.model} costs $\ ${element.price}`)
// const list = cars.map(element => "This " + element.brand + " from " + element.model + " costs $" + element.price)
console.log(list);

// BMW Sum
// Easy to read Answer
const bmw = cars.filter(element => element.brand === 'BMW')
const bmwPrice = bmw.map(el => el.price)
const reducer = (acc, cur) => acc + cur
const bmwSum = bmwPrice.reduce(reducer)
console.log(bmwSum);

// Direct answer
// const bmwSum = cars.filter(element => element.brand === 'BMW').map(el => el.price).reduce(reducer)